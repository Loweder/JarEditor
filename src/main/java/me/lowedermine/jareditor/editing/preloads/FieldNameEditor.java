...
import me.lowedermine.jareditor.jar.descriptors.ObjectDescriptorType;
...
public abstract class FieldNameEditor extends NameEditor<ClassFieldInfo, String> {
    ...
    protected final void setMappings(Map<ClassFieldInfo, String> mappings) {
        ...
        this.mappings.forEach((k, v) -> unMappings.put(new ClassFieldInfo(k.clazz, new FieldInfo(v, (FieldDescriptor) k.desc)), k.name));
    }
    ...
    private void mapElementValue(ElementValuePair.ElementValue elementValue) {
        ...
        switch (elementValue.type) {
            case ENUM:
                ((FieldInfo)value).name = map(new ClassFieldInfo(((ObjectDescriptorType)((FieldDescriptor)((FieldInfo) value).desc).type), new FieldInfo(((FieldInfo) value).name, (FieldDescriptor) ((FieldInfo) value).desc)));
                break;
            case ANNOTATION:
